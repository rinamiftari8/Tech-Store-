  ...
  <div class="forms">
    <form name="editoProduktin" onsubmit="" action='' method="POST" enctype="multipart/form-data">
      ...
      ?><select name="kategoria" class="dropdown">
        ...
        ?>
        <option selected hidden value="<?php echo $produkti['kategoriaID'] ?>"><?php echo $produkti['emriKategoris'] ?>
        </option>
      </select>
      <?php
      ?>
      <input class="form-input" name="pdName" type="text" placeholder="Emri i Produktit"
        value="<?php echo htmlspecialchars($produkti['emriProduktit'], ENT_QUOTES, 'UTF-8') ?>" required>
      <input class="form-input" name="cmimiPd" type="number" step="0.01" placeholder="Cmimi Produktit"
        value="<?php echo htmlspecialchars($produkti['qmimiProduktit'], ENT_QUOTES, 'UTF-8') ?>" required>
      <input class="form-input" name="pdPhoto" type="file" placeholder="Foto e Produktit" accept="image/*">
      <textarea placeholder="Pershkrimi Produktit"
        name="pershkrimiProd"><?php echo htmlspecialchars($produkti['pershkrimiProd'], ENT_QUOTES, 'UTF-8') ?></textarea>
      <div>
