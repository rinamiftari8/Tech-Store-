...
$nrPorosis = isset($_GET['nrPorosis']) && ctype_digit($_GET['nrPorosis']) ? (int)$_GET['nrPorosis'] : 0;
if ($nrPorosis === 0) {
  header('Location: ../userPages/porosit.php');
  exit();
}

$porosiaCRUD->setPorosiaID($nrPorosis);

$porosia = $porosiaCRUD->shfaqPorosinSipasID();
$produktetEPorosis = $porosiaCRUD->shfaqProduktetEPorosisSipasID();
$allowed = isset($_SESSION['userID']) && ($_SESSION['aksesi'] != 0 || $porosia['idKlienti'] === (int) $_SESSION['userID']);
if (!$allowed) {
  header('Location: ../userPages/porosit.php');
  exit();
}
